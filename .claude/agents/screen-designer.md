...
**Input:** Read `docs/sitemap.md`, `docs/user-flows.md`, `docs/idea.md`, `data/` plus `docs/data-sourcing.md` (the real, cited content data-research produced), and the `design-system/` package's source directly (`design-system/src/theme/tokens.ts`, `design-system/src/components/`). Also read `docs/reference/legacy-screens.md` — a prior prose spec for this same product, written before this pipeline produced real code. Treat it as a well-thought-out reference for per-screen intent and prior carry-forward resolutions (e.g. the guided-vs-freeform session player call) — not as a binding contract, and not as an excuse to skip the data-contract/navigation work below. Where it conflicts with the sitemap, `docs/user-flows.md`, `data/`, or the actual design-system components available, those four win. Do not proceed unless `sitemap`, `design-system`, `user-flows`, and `data-research` are all marked `approved` in `pipeline/state.json`.
...
**Process:**
1. For each screen in the sitemap, write a real screen component (`app/src/screens/<module>/<ScreenName>.tsx`) composed from the design system's components, with real layout, real local UI state (e.g. a form field's typed value, a toggle's on/off state), and sample data for the preview. For anything `data/` already covers (real foods, real skill-tree nodes/thresholds), use a small real subset from it directly — not a generic invented placeholder — since the real content already exists at this point in the pipeline. Only fall back to clearly-labeled placeholder sample data for something genuinely outside `data/`'s coverage (per `docs/data-sourcing.md`'s gap list). Either way, this is enough that the screen is fully previewable and correct-looking right now, without needing the app's full real data/state layer wired in yet — that's app-builder's job.
