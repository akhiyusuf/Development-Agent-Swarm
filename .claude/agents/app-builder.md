...
**Input:** Read `docs/sitemap.md`, `research/product-research.md` (for the business-logic rules it specifies), `docs/screens.md` (for each screen's documented data contract), and `data/` plus `docs/data-sourcing.md` (the real, cited nutrition and workout-progression content data-research produced). Read the actual screen files in `app/src/screens/` to see exactly what shape of data/callbacks each one expects. Do not proceed unless both `screens` and `data-research` are marked `approved` in `pipeline/state.json`.
...
2. Build the data layer in `app/src/data/` around the **real, cited content data-research produced** in `data/` — do not build your own mock/seeded/placeholder food or skill-tree data; import and adapt the real dataset. If `docs/data-sourcing.md` flags a genuine coverage gap (a food, region, or skill dimension it didn't source), keep that specific gap clearly-labeled as a known gap rather than inventing content to fill it silently — but this should be the exception, not the default posture. If a screen's sample data (from screen-designer) was a generic placeholder pending real data, replace it with the real dataset now.
